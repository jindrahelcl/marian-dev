///// THIS FILE IS COPIED FROM ENCODER_CLASSIFIER.
...
public:
  ...
  virtual Ptr<CTCDecoderResult> apply(Ptr<ExpressionGraph>, Ptr<data::CorpusBatch>, bool) = 0;
...
public:
  ...
  virtual Ptr<CTCDecoderResult> apply(Ptr<ExpressionGraph> graph, Ptr<data::CorpusBatch> batch, bool clearGraph) override {
    if(clearGraph)
      clear(graph);
  ...
  virtual Logits build(Ptr<ExpressionGraph> graph,
    ...
    auto decoderResult = apply(graph, batch, clearGraph);
    // returns raw logits
    return Logits(decoderResult->getLogProbs());
  }
